/****************************************************************************
 ...
 When           Who     What/Why
 -------------- ---     --------
 04/02/16 15:27 jec     started port to Arduino
...
#define ES_READ_FLASH_BYTE(_flash_var_)    pgm_read_byte(_flash_var_)                  
#define ES_READ_FLASH_WORD(_flash_var_)    pgm_read_word(_flash_var_)                  
...
extern volatile uint8_t tempSREG_;

#define EnterCritical()	do { \
    (tempSREG_) = SREG; \
    cli(); \
} while (0)
#define ExitCritical() (SREG = (tempSREG_))
...
#define IsNewKeyReady()  ( Serial.available() != 0 )
#define GetNewKey()      Serial.read()
